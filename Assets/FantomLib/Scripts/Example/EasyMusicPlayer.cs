...
#region Properties and Local values Section
        ...
        private int index = 0;                      //Current index in playlist
        ...
        private void PlayClip(int idx)
        {
            ...
            if (!audioSource.isPlaying || audioSource.clip != playlist[idx].clip)
            {
                ...
                if (OnSongChanged != null)
                    OnSongChanged.Invoke(playlist[idx]);
            }
        ...
        private bool AutoSettingCheckAndExecute()
        {
            ...
            if (TargetSongCount == 0)
            {
                ...
                if (autoSettingOnError)
                {
                    ...
                    mes = localize.errorSongNotFound.TextByLanguage(localize.language)
                        + "\n" + localize.optionalAutoExceptPresets.TextByLanguage(localize.language);
                }
                else
                {
                    mes = localize.errorSongNotFound.TextByLanguage(localize.language)
                        + "\n" + localize.confirmSetting.TextByLanguage(localize.language);
                }
                ShowToast(mes);
...
#region Load AudioClip Section
        ...
        IEnumerator LoadAudio(string path, int idx, Action<int, int> completeCallback, int dir = 0)
        {
            ...
            if (!File.Exists(path)) {
                string mes = localize.errorNotExist.TextByLanguage(localize.language)
                    + "\n" + path;
                if (autoPlaylistOnError)
                {
                    RemoveSongOnError(idx);
                    mes += "\n" + localize.optionalAutoPlaylist.TextByLanguage(localize.language);
                }
                ShowToast(mes, true);
            ...
            {
                string mes = localize.errorLoadFailure.TextByLanguage(localize.language)
                    + "\n" + path;
                if (autoPlaylistOnError)
                {
                    RemoveSongOnError(idx);
...
#pragma warning disable 0219    //'items' is never used. But it is used at runtime.
        ...
        public void OpenSongDialog()
        {
            ...
            CheckItem checkExceptPresets = new CheckItem(localize.exceptPresetsCaption.TextByLanguage(localize.language), KEY_EXCEPT_PRESETS, exceptPresets, 0);
            TextItem textExceptPresets = new TextItem(localize.exceptPresetsExplanation.TextByLanguage(localize.language), Color.red);

            CheckItem checkAutoPlaylist = new CheckItem(localize.autoPlaylistCaption.TextByLanguage(localize.language), KEY_AUTO_PLAYLIST, autoPlaylistOnError, 0);
            CheckItem checkAutoSetting = new CheckItem(localize.autoSettingCaption.TextByLanguage(localize.language), KEY_AUTO_SETTING, autoSettingOnError, 0);

            SwitchItem switchResetPlaylist = new SwitchItem(localize.resetPlaylistCaption.TextByLanguage(localize.language), KEY_RESET_PLAYLIST, false, Color.blue);
            SwitchItem switchResetSetting = new SwitchItem(localize.resetSettingCaption.TextByLanguage(localize.language), KEY_RESET_SETTING, false, Color.blue);

            TextItem textReset = new TextItem(localize.resetExplanation.TextByLanguage(localize.language), Color.red);
            ...
            AndroidPlugin.ShowCustomDialog(
                localize.songDialogTitle.TextByLanguage(localize.language), "",
                items, gameObject.name, "OnReceiveResult", false, 
                localize.songDialogOK.TextByLanguage(localize.language), 
                localize.songDialogCancel.TextByLanguage(localize.language));
#endif
        }
...
#pragma warning restore 0219    //'items' is never used. But it is used at runtime.
            ...
            if (!string.IsNullOrEmpty(message))
            {
                ...
                if (param != null)
                {
                    ...
                    if (resetSetting && resetPlaylist)
                    {
                        ...
                        ShowToast(localize.messageResetBoth.TextByLanguage(localize.language));
                    }
                    else if (resetSetting)
                    {
                        ResetSetting();
                        ShowToast(localize.messageResetSetting.TextByLanguage(localize.language));
                    }
                    else
                    {
                        if (resetPlaylist)
                        {
                            ResetPlaylist();
                            ShowToast(localize.messageResetPlaylist.TextByLanguage(localize.language));
                        }
                ...
                if (param != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        if (idx >= 0)
                        {
                            ...
                            ShowToast(localize.errorNotExist.TextByLanguage(localize.language));
                    }
                }
            }
        }
        ...
        public void AddSong(AudioInfo info)
        {
            ...
            {
                ShowToast(localize.errorNotGetPath.TextByLanguage(localize.language));
                return;
            }
            ...
            {
                ShowToast(localize.errorNotAvailableType.TextByLanguage(localize.language));
                return;
            }
            ...
            if (idx >= 0)
            {
                ShowToast(localize.errorDuplicateSong.TextByLanguage(localize.language));
                return;
            }
        ...
        public void OpenRemoveDialog()
        {
            ...
            if (songKeys.Length > 0)
            {
                ...
                AndroidPlugin.ShowMultiChoiceDialog(
                    localize.removeDialogTitle.TextByLanguage(localize.language),
                    songTitles, null,
                    gameObject.name, "ReceiveRemoveResult", songKeys,
                    localize.removeDialogOK.TextByLanguage(localize.language),
                    localize.removeDialogCancel.TextByLanguage(localize.language));
#endif
            }
            else
            {
                ShowToast(localize.errorRemoveNotFound.TextByLanguage(localize.language));
            }
        }
        ...
        public void RemoveSong(string[] paths)
        {
            ...
            if (deleted > 0)
                ShowToast(string.Format(localize.messageRemoved.TextByLanguage(localize.language), deleted));
